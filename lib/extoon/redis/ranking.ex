defmodule Extoon.Redis.Ranking do
  defmodule Base do
    use Rdtype,
      uri: Application.get_env(:extoon, :redis)[:ranking],
      coder: Extoon.Redis.Json,
      type: :sorted_set
  end
  ...
  def beginning_of_biannual({{year, month, _}, _}) do
    case month do
      m when m in 1..6  -> Timex.beginning_of_month Timex.DateTime.from({year, 1, 1})
      m when m in 7..12 -> Timex.beginning_of_month Timex.DateTime.from({year, 6, 1})
    end
  end
  def end_of_biannual({{year, month, _}, _}) do
    case month do
      m when m in 1..6  -> Timex.end_of_month Timex.DateTime.from({year, 6, 1})
      m when m in 7..12 -> Timex.end_of_month Timex.DateTime.from({year, 12, 1})
    end
  end
end
